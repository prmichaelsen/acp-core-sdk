...
### Added
- Version Management (Task 22, M4)
  - Git tag `v1.21.0` created for this release
  - `docs/README.md` version reference updated to v1.20.0+ → v1.21.0+
  - Version consistency verified across `package.yaml`, `CHANGELOG.md`, `docs/README.md`

### Fixed
- Package Validation (Task 21, M4)
  - `README.md`: replaced 3 placeholder sections with real content:
    - "Why Use This Package" — 5 concrete benefits
    - "Usage" — install command, multi-target deployment diagram, guide links
    - "Dependencies" — table of runtime dependencies by adapter type

## [1.20.0] - 2026-02-27

### Added
- Test Coverage Validation (Task 20, M3) — unit tests and coverage configuration
  - `src/services/user.service.spec.ts` — 18 test cases covering all `UserService` methods:
    - `findUser`: returns `Ok<UserDTO>` for known ID, `Err<NotFoundError>` for unknown, handles empty store
    - `createUser`: persists user, defaults role to `member`, trims whitespace, validates email/name, detects conflicts
    - `listUsers`: returns all, filters by role, respects `limit`, defaults limit to 20, caps at 100, paginates with cursor, handles empty store
    - `parseUserId`: accepts valid IDs, rejects empty string / whitespace-only / non-empty strings with strict format
  - `config/jest.config.js` — coverage thresholds (80% global, 90% for `src/services/`), coverage reporters (`text`, `lcov`, `html`), `!src/testing/**` excluded from coverage collection

### Milestone
- **M3 Documentation & Testing: COMPLETE** (7/7 tasks)
